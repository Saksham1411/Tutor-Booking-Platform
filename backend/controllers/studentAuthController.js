...
const loginStudent = async (req, res) => {
  ...
  if (!student)
    return res.status(StatusCodes.UNAUTHORIZED).send("Student not find");

  //matching password
  const isMatched = bcrypt.compareSync(password, student.password);
  if (!isMatched)
    return res.status(StatusCodes.UNAUTHORIZED).send("Password not match");

  //creating jwt token
  const token = jwt.sign(
    { id: student._id, name: student.name, email },
    process.env.JWT_SECRET
  );
  student.password = undefined;
  student = {
